...
public class CommandManager {
    ...
    public void handleCommand(String string) {
        ...
        if (params.get(0).isEmpty()) {
            ChatUtil.sendFormatted(String.format("%sUnknown command&r", OpenSkid.clientName).replace("&", "§"));
        } else {
            ...
            ChatUtil.sendFormatted(String.format("%sUnknown command (&o%s&r)&r", OpenSkid.clientName, params.get(0)).replace("&", "§"));
        }
    }
