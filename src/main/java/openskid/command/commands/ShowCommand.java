    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() < 2) {
            ...
            if (module == null) {
                ...
                ChatUtil.sendFormatted(String.format("%s&o%s&r is not hidden in HUD&r", OpenSkid.clientName, module.getName()));
            } else {
                module.setHidden(false);
                ChatUtil.sendFormatted(String.format("%s&o%s&r is no longer hidden in HUD&r", OpenSkid.clientName, module.getName()));
            }
        } else {
            for (Module module : OpenSkid.moduleManager.allModules()) {
                module.setHidden(false);
            }
            ChatUtil.sendFormatted(String.format("%sAll modules are no longer hidden in HUD&r", OpenSkid.clientName));
        }
    }
}
