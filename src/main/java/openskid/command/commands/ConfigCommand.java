    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() < 2) {
            ...
            ChatUtil.sendFormatted(
                    String.format("%sUsage: .%s &oload&r/&osave&r <&oname&r> | .%s &olist&r | .%s &ofolder&r", OpenSkid.clientName, command, command, command)
            );
        } else {
            ...
            switch (sub) {
                ...
                case "reload":
                    if (args.size() < 3) {
                        ChatUtil.sendFormatted(
                                String.format("%sMissing config name (use '&odefault&r' or '&o!&r' to load default config)&r", OpenSkid.clientName)
                        );
                        return;
                    }
                ...
                case "list":
                    try {
                        File[] configs = new File("./config/OpenSkid/").listFiles(FILE_FILTER);
                        if (configs == null) {
                            throw new Exception();
                        }
                        if (configs.length == 0) {
                            ChatUtil.sendFormatted(String.format("%sNo configs found (&o%s&r)&r", OpenSkid.clientName, "./config/OpenSkid/"));
                        }
                        Arrays.sort(configs, LastModifiedFileComparator.LASTMODIFIED_REVERSE);
                        ChatUtil.sendFormatted(String.format("%sConfigs:&r", OpenSkid.clientName));
                        ...
                        ChatUtil.sendFormatted(String.format("%sFailed to read (&o%s&r)&r", OpenSkid.clientName, "./config/OpenSkid/"));
                    }
                    return;
                case "f":
                case "folder":
                ...
                case "directory":
                    try {
                        Desktop.getDesktop().open(new File("./config/OpenSkid/"));
                    } catch (Exception e) {
                        ChatUtil.sendFormatted(String.format("%sFailed to open (&o%s&r)&r", OpenSkid.clientName, "./config/OpenSkid/"));
                    }
                    return;
                default:
