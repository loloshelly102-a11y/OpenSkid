...
public class CardClickGUI extends GuiScreen {
    ...
    private static CardClickGUI instance;
    ...
    static {
        map(Tab.GHOST, "AimAssist", "AutoClicker", "Reach", "Velocity", "WTap", "Wtap", "HitBox",
                "HitSelect", "JumpReset", "BackTrack", "Hitflick", "MoreKB", "KnockbackDelay", "ClickAssits", "SprintReset", "BlockHit",
                "SmartClicking", "AutoRod");
        map(Tab.COMBAT, "KillAura", "TargetStrafe", "NoHitDelay", "AntiFireball", "LagRange", "Refill",
                "Criticals", "Displace", "ServerLag", "AutoBlock", "BurstClicker",
        ...
        map(Tab.RENDER, "ESP", "Chams", "FullBright", "Fullbright", "Tracers", "NameTags", "Xray",
                ...
                "HitParticleEffects", "KillEffect", "KillSounds", "DynamicIsland", "ESP2D", "TeamHealthDisplay", "Statistics",
                "Animations", "RenderFixes", "ClickGUI", "ClickGui", "FreeLook",
                "BedPlates", "FKCounter", "ClosestPlayerHUD", "LeapModeHUD", "MegaWallsDetector",
                "FireBallPredict", "DamageTags", "TNTTimer", "PotionHUD", "Arrows", "BlocksESP", "MobESP", "ItemTags", "FallIndicator", "InventoryHUD", "KeyStrokes", "EntityCulling");
        map(Tab.EXPLOIT, "Disabler", "ClientSpoofer", "ModSpoofer", "ExploitFixer", "AntiFalseFlag",
                "GhostBlock", "PingSpoof", "NoRotate", "AntiObfuscate", "Blink", "Timer",
                "TimerRangev999", "AntiVoid", "FakeLag", "GhostHand", "BedNuker", "BedTracker");
        map(Tab.MISC, "StaffDetector", "MurderDetector", "BedProximityAlert", "CheatDetector",
                "AutoPlay", "AutoGG", "AutoReconnect", "Panic", "KillSults",
                "KillMessage", "ViewPackets", "AutoBuy", "PartyDetector", "PlayerList",
                "AutoRequeue", "AutoWho", "BedWars", "BridgeInfo", "DuelsStats",
                "MurderMystery", "SumoFences", "SkyWars", "WoolWars", "SpeedBuilders", "SkywarsAlerts", "ThePitUtils");
    }
    ...
    private final Map<Tab, List<CardModuleCard>> moduleCards = new EnumMap<Tab, List<CardModuleCard>>(Tab.class);
    private final List<CardModuleCard> allCards = new ArrayList<CardModuleCard>();
    private final List<CardModuleCard> searchResults = new ArrayList<CardModuleCard>();
    ...
    private final CardCaSScreen configScreen = new CardCaSScreen();
    ...
    public static CardClickGUI getInstance() {
        if (instance == null) instance = new CardClickGUI();
        return instance;
    }
    ...
    public static Color accent() {
        if (OpenSkid.moduleManager == null) return new Color(0x4FC3F7);
        ClickGUIModule module = (ClickGUIModule) OpenSkid.moduleManager.getModule("ClickGUI");
    ...
    private void rebuildModuleCache() {
        ...
        for (Tab tab : Tab.values()) {
            if (tab.moduleTab) moduleCards.put(tab, new ArrayList<CardModuleCard>());
        }

        List<Module> modules = new ArrayList<Module>(OpenSkid.moduleManager.allModules());
        ...
        for (Module module : modules) {
            CardModuleCard card = new CardModuleCard(module);
            allCards.add(card);
            Tab tab = resolveModuleTab(module);
            List<CardModuleCard> list = moduleCards.get(tab);
            if (list == null) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.drawRoundedRect(windowX, windowY, WINDOW_W, WINDOW_H, ROUND,
                CardColors.withAlpha(CardColors.BACKGROUND, (int) (254 * opacityAnimation)).getRGB(),
                true, true, true, true);
        drawWindowChrome();
    ...
    private void drawTransition(float deltaTime) {
        ...
        RenderUtil.drawRoundedRect(windowX, windowY, WINDOW_W, WINDOW_H, ROUND,
                CardColors.withAlpha(CardColors.BACKGROUND, (int) (transitionAlpha * 210)).getRGB(),
                true, true, true, true);
    }
    ...
    private void drawSidebar(int mouseX, int mouseY, float deltaTime) {
        ...
        RenderUtil.drawRoundedRect(windowX, windowY, SIDEBAR_W, WINDOW_H, ROUND,
                CardColors.withAlpha(CardColors.SECONDARY, (int) (255 * opacityAnimation)).getRGB(),
                true, false, false, true);

        if (FontManager.productSans24 != null) {
            FontManager.productSans24.drawString("OpenSkid", windowX + 14, windowY + 12,
                    CardColors.withAlphaRGB(Color.WHITE, (int) (255 * opacityAnimation)));
        }
        if (FontManager.productSans12 != null) {
            float nameWidth = FontManager.productSans24 != null
                    ? (float) FontManager.productSans24.getStringWidth("OpenSkid") : 38f;
            FontManager.productSans12.drawString("Card", windowX + 18 + nameWidth, windowY + 14,
                    CardColors.withAlpha(accent, (int) Math.min(200, 255 * opacityAnimation)).getRGB());
        }
        ...
        for (int i = 0; i < tabs.length; i++) {
            ...
            if (FontManager.productSans16 != null) {
                FontManager.productSans16.drawString(tab.label, itemX + 11,
                        ...
                        CardColors.withAlphaRGB(Color.WHITE, textAlpha));
            }
            if (tab.moduleTab && FontManager.productSans12 != null) {
                List<CardModuleCard> cards = moduleCards.get(tab);
                ...
                FontManager.productSans12.drawString(count, itemX + itemW - countW - 8,
                        ...
                        CardColors.withAlphaRGB(CardColors.TEXT_TRINARY, textAlpha));
            }
        }
    }
    ...
    private void drawSearchScreen(float x, float y, float w, float h, int mouseX, int mouseY,
        ...
        int color = searchText.isEmpty() ? CardColors.TEXT_TRINARY.getRGB() : CardColors.TEXT.getRGB();
        if (FontManager.productSans16 != null) {
    ...
    private void drawModuleList(List<CardModuleCard> cards, float x, float y, float w, float h, int mouseX, int mouseY,
    ...
    private void drawThemeScreen(float x, float y, float w, float h, int mouseX, int mouseY) {
        ...
        if (FontManager.productSans20 != null) {
            FontManager.productSans20.drawString("Themes", x + 7, y + 7, CardColors.TEXT.getRGB());
        }
        if (FontManager.productSans12 != null) {
            FontManager.productSans12.drawString("Accent palette", x + 7, y + 25, CardColors.TEXT_TRINARY.getRGB());
        }
    ...
    private void drawLanguageScreen(float x, float y, float w, float h, int mouseX, int mouseY) {
        ...
        if (FontManager.productSans20 != null) {
            FontManager.productSans20.drawString("Language", x + 7, y + 7 - screenScrollOffset, CardColors.TEXT.getRGB());
        }
        if (FontManager.productSans12 != null) {
            FontManager.productSans12.drawString("Visual selector", x + 7, y + 25 - screenScrollOffset,
                    CardColors.TEXT_TRINARY.getRGB());
        }
        ...
        for (int i = 0; i < languages.length; i++) {
            ...
            if (FontManager.productSans16 != null) {
                FontManager.productSans16.drawString(languages[i], x + 25, rowY + 7, CardColors.TEXT.getRGB());
            }
        }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        List<CardModuleCard> cards = currentCards();
        if (cards != null) {
            for (CardModuleCard card : cards) {
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state) {
        dragging = false;
        for (CardModuleCard card : allCards) {
            card.released();
        }
    }
    ...
    private List<CardModuleCard> currentCards() {
    ...
    private void handleInvWalk() {
        try {
            Module invWalk = OpenSkid.moduleManager.getModule("InvWalk");
