...
public abstract class CardValueEditor {

    protected final Property<?> property;

    protected CardValueEditor(Property<?> property) {
    ...
    protected int alpha(Color color, int opacity) {
        return CardColors.withAlpha(color, opacity).getRGB();
    }
    ...
    public static class BooleanEditor extends CardValueEditor {
        ...
        public void draw(float x, float y, float w, int mouseX, int mouseY,
            ...
            if (FontManager.productSans16 != null) {
                FontManager.productSans16.drawString(prop.getName(), x, y + 4,
                        CardColors.withAlphaRGB(CardColors.TEXT_SECONDARY, opacity));
    ...
    public static class ModeEditor extends CardValueEditor {
    ...
    public static class SliderEditor extends CardValueEditor {
        ...
        public void draw(float x, float y, float w, int mouseX, int mouseY,
            ...
            RenderUtil.drawRoundedRect(thumbX - thumb / 2f, trackY + 2 - thumb / 2f, thumb, thumb, thumb / 2f,
                    CardColors.withAlphaRGB(Color.WHITE, opacity), true, true, true, true);
        }
        ...
        private void setNumber(double value) {
            ...
            if (property instanceof IntProperty) {
                ...
                ((LongProperty) property).setValue(Math.round(value));
            }
        }
        ...
        private String valueText() {
            ...
            if (property instanceof PercentProperty) return ((PercentProperty) property).getValue() + "%";
            if (property instanceof LongProperty) return String.valueOf(((LongProperty) property).getValue());
            return "";
        }
    }

    public static class ColorEditor extends CardValueEditor {
        ...
        public void draw(float x, float y, float w, int mouseX, int mouseY,
            ...
            RenderUtil.drawCircleOutline(pickerDotX, pickerDotY, 3.5f, 1.2f, CardColors.withAlphaRGB(Color.WHITE, opacity));
            ...
            if (FontManager.productSans12 != null) {
                ...
                FontManager.productSans12.drawString(hex, x + w - (float) FontManager.productSans12.getStringWidth(hex),
                        pickerY + pickerH + 9, CardColors.withAlphaRGB(CardColors.TEXT_TRINARY, opacity));
            }
    ...
    public static class TextEditor extends CardValueEditor {
        ...
        public void draw(float x, float y, float w, int mouseX, int mouseY,
            ...
            if (FontManager.productSans12 != null) {
                ...
                FontManager.productSans12.drawString(trimToWidth(text, fieldW - 8), fieldX + 4, y + 6,
                        CardColors.withAlphaRGB(Color.WHITE, opacity));
            }
        }
    ...
    public static class ReadOnlyEditor extends CardValueEditor {
        public ReadOnlyEditor(Property<?> prop) {
            super(prop);
        }
        ...
        public void draw(float x, float y, float w, int mouseX, int mouseY,
                         float partialTicks, float deltaTime, int opacity) {
            if (FontManager.productSans16 != null) {
                String coords = property instanceof DragProperty
                        ? Math.round(((DragProperty) property).position.x) + ", " + Math.round(((DragProperty) property).position.y)
                        : String.valueOf(property.getValue());
                FontManager.productSans16.drawString(property.getName(), x, y + 4,
                        CardColors.withAlphaRGB(CardColors.TEXT_SECONDARY, opacity));
                String value = coords;
                float valueWidth = (float) FontManager.productSans16.getStringWidth(value);
                FontManager.productSans16.drawString(value, x + w - valueWidth, y + 4,
                        CardColors.withAlphaRGB(CardColors.TEXT_TRINARY, opacity));
            }
        }
