...
import openskid.ui.impl.clickgui.legacy.callback.GuiInput;
...
public class TextComponent implements Component {
    ...
    public void draw(AtomicInteger offset) {
        ...
        OpenSkid.fontManagers.getFont(24).drawString(this.property.getName().replace("-", " ") + ": " + ChatColors.formatColor(this.property.formatValue()), (float) ((this.module.category.getX() + 4) * 2), (float) ((this.module.category.getY() + this.offsetY + 5) * 2), -1, false);
        GL11.glPopMatrix();
    }
    ...
    public void onClick(int x, int y, int mouse) {
        if (this.isHovered(x, y) && mouse == 0 && this.module.isOpened) {
            GuiInput.prompt(property.getName().replace("-", " "), property.getValue(), property::setValue, LegacyClickGui.getInstance());
        }
    }
