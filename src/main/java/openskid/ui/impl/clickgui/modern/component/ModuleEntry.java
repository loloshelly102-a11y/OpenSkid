package openskid.ui.impl.clickgui.modern.component;
...
import openskid.ui.impl.clickgui.modern.MaterialTheme;
...
public class ModuleEntry extends Component {
    ...
    private final Map<Component, Float> rowHover = new HashMap<>();
        ...
        if (currentSettingsHeight > 1.0f) {
            ...
            for (int i = 0; i < propertiesComponents.size(); i++) {
                ...
                if (relativeY < currentSettingsHeight) {
                    ...
                    int rowScrolledY = comp.getY() - scrollOffset;
                    boolean rowHovered = mouseX >= comp.getX() && mouseX <= comp.getX() + comp.getWidth()
                            && mouseY >= rowScrolledY && mouseY <= rowScrolledY + comp.getHeight();
                    Float prevHover = rowHover.get(comp);
                    float rowAnim = AnimationUtil.animateSmooth(rowHovered ? 1.0f : 0.0f,
                            prevHover == null ? 0.0f : prevHover, 10.0f, deltaTime);
                    rowHover.put(comp, rowAnim);
                    if (rowAnim > 0.01f && alpha > 5) {
                        int rowBg = MaterialTheme.getRGBWithAlpha(MaterialTheme.SURFACE_CONTAINER_HIGH,
                                (int) (alpha * 0.6f * rowAnim));
                        RenderUtil.drawRoundedRect((float) comp.getX(), (float) rowScrolledY,
                                (float) comp.getWidth(), (float) comp.getHeight(), 4.0f, rowBg,
                                true, true, true, true);
                    }
