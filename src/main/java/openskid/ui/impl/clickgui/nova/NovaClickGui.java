package openskid.ui.impl.clickgui.nova;
...
import openskid.font.impl.UFontRenderer;
import openskid.module.Module;
import openskid.module.modules.HUD;
import openskid.property.Property;
import openskid.property.properties.*;
import openskid.ui.callback.GuiInput;
import openskid.ui.impl.clickgui.legacy.dataset.Slider;
...
public class NovaClickGui extends GuiScreen {
    private static NovaClickGui instance;
    private final File configFile = new File("./config/OpenSkid/", "clickgui_nova.txt");
    ...
    private static final Set<String> COMBAT = set(
            ...
            "HitSelect", "JumpReset", "BackTrack", "Hitflick", "TimerRange", "ClickAssits", "Criticals", "BlockHit", "AutoBlock",
            "SprintReset", "Displace", "TickBase", "Piercing", "Stasis", "SmartClicking", "AutoRod",
            "ForwardTrack", "KeepRange", "SmartBlinker", "LegitReach", "ProjectileAimBot", "BurstClicker", "ArmorBreaker");
    ...
    private static final Set<String> RENDER = set(
            ...
            "HitParticleEffects", "KillEffect", "KillSounds", "DynamicIsland", "ESP2D", "TeamHealthDisplay", "Statistics", "Animations",
    ...
    private static UFontRenderer font() {
        return OpenSkid.fontManagers.getFont(16);
    }
    ...
    public static NovaClickGui getInstance() {
        return instance;
    }
        ...
        for (CategoryComponent category : categoryList) {
            ...
            for (Component module : category.getModules()) {
                if (module instanceof ModuleComponent) {
                    ((ModuleComponent) module).renderTooltip(x, y);
                }
            }
        }
    }
    ...
    public void onGuiClosed() {
        ...
        if (guiModule instanceof openskid.module.modules.ClickGUIModule && ((openskid.module.modules.ClickGUIModule) guiModule).isSwitchingGuiStyle()) {
            return;
        }
        if (guiModule != null) {
    ...
    private class ModuleComponent implements Component {
            ...
            if (!OpenSkid.propertyManager.properties.get(mod).isEmpty()) {
                for (Property<?> baseProperty : OpenSkid.propertyManager.properties.get(mod)) {
                    if (baseProperty instanceof BooleanProperty) {
                        ...
                        SliderComponent c = new SliderComponent(new Slider() {
                            @Override
                        ...
                        Component c = new Component() {
                            ...
                            public void draw(AtomicInteger offset) {
                                ...
                                font().drawString(text, parent.category.getX() + 5, parent.category.getY() + posY[0] + 4, new Color(170, 170, 170).getRGB());
                            }
        ...
        public void update(int mousePosX, int mousePosY) {
            if (category.isOpened() && isHovered(mousePosX, mousePosY)) {
        ...
        public void renderTooltip(int x, int y) {
            if (!category.isOpened() || !isHovered(x, y)) return;
            ...
            int pad = 4;
            int tw = font().getStringWidth(desc);
            int th = font().getHeight();
            int bw = tw + pad * 2;
            int bh = th + pad * 2;
            int bx = x + 12;
            int by = y + 12;
            if (bx + bw > width - 2) bx = x - bw - 8;
            if (by + bh > height - 2) by = y - bh - 8;
            if (bx < 2) bx = 2;
            if (by < 2) by = 2;
            Gui.drawRect(bx, by, bx + bw, by + bh, new Color(20, 20, 25, 250).getRGB());
            font().drawString(desc, bx + pad, by + pad, new Color(200, 200, 200).getRGB());
        }
    ...
    private class BindComponent implements Component {
        ...
        public void draw(AtomicInteger offset) {
            ...
            this.renderText(this.isBinding ? "Press a key..." : "Bind" + ": " + Keyboard.getKeyName(this.parentModule.mod.getKey()),
                    ((HUD) OpenSkid.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis(), offset.get()));
            GL11.glPopMatrix();
        }
    ...
    private class ModeComponent implements Component {
        ...
        public void draw(AtomicInteger offset) {
            ...
            if (changeAnimation > 0.01F) {
                int accentColorInt = ((HUD) OpenSkid.moduleManager.modules.get(HUD.class))
    ...
    private class TextComponent implements Component {
        ...
        public void mouseDown(int x, int y, int button) {
            if (this.isHovered(x, y) && button == 0 && this.module.panelExpand) {
                GuiInput.prompt(property.getName().replace("-", " "), property.getValue(), property::setValue, NovaClickGui.getInstance());
            }
        }
