...
public class Radar extends Module {
    ...
    public Radar() {
        super("Radar", false, false, "Shows nearby players as dots on a circular radar.");
    }
    ...
    private Color getEntityColor(EntityPlayer entityPlayer) {
        if (TeamUtil.isFriend(entityPlayer)) {
            ...
            switch (this.colorMode.getValue()) {
                ...
                case 2:
                    HUD hudColor = (HUD) OpenSkid.moduleManager.modules.get(HUD.class);
                    int color = (hudColor != null && hudColor.isEnabled()) ? hudColor.getColor(System.currentTimeMillis()) : Color.WHITE.getRGB();
    ...
    public void onRender(Render2DEvent event) {
        ...
        float hudScale = (hud != null && hud.isEnabled()) ? hud.scale.getValue() : 1.0F;
        boolean hudShadow = hud != null && hud.isEnabled() && hud.shadow.getValue();
        ...
        if (this.showPVP.getValue()) {
            ...
            GlStateManager.scale(hudScale / 2, hudScale / 2, 1.0f);
    ...
    public void drawRadarCircle(double x, double y, double angle, double radius,
        ...
        if ((crossColor >>> 24) != 0) {
            ...
            GlStateManager.blendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
            HUD hud = (HUD) OpenSkid.moduleManager.modules.get(HUD.class);
            int color = (hud != null && hud.isEnabled()) ? hud.getColor(System.currentTimeMillis()) : Color.WHITE.getRGB();
            boolean shadow = hud != null && hud.isEnabled() && hud.shadow.getValue();
