...
import openskid.property.properties.ItemListProperty;
import openskid.property.properties.ModeProperty;
import openskid.util.CombatTargeting;
 ...
 * @author strangerrrrs
 */
// Void scan adapted from donor.jar_Decompiler.com/keystrokesmod/module/impl/combat/Displace.java
public class Displace extends Module {
    ...
    public final ModeProperty mode = new ModeProperty("Mode", 0, new String[] {"Displace", "Off"});
    public final IntProperty scanDirs =
            new IntProperty("Scan-dirs", 32, 8, 64, () -> mode.getValue() == 0);
    public final IntProperty scanRings =
            new IntProperty(
                    "Scan-rings",
                    12,
                    1,
                    16,
                    () -> mode.getValue() == 0 && dynamicAngle.getValue() == 0);
    public final FloatProperty scanRange =
            new FloatProperty(
                    "Scan-range", 6.0F, 2.0F, 10.0F, () -> mode.getValue() == 0 && dynamicAngle.getValue() == 1);
    public final IntProperty fadeMs =
            new IntProperty("Fade-ms", 250, 0, 1000, () -> mode.getValue() == 0);
    ...
    public Displace() {
        super("Displace", false, false, "Strafes targets toward nearby void while attacking.");
    }

    @Override
    public String[] getSuffix() {
        if (mode != null && mode.getValue() == 1) {
    ...
    private int currentScanDirs() {
        if (scanDirs == null) {
            return VOID_SCAN_DIRECTIONS;
        }
        return Math.max(1, Math.min(64, scanDirs.getValue()));
    }

    private int currentScanRings() {
        if (scanRings == null) {
            return VOID_SCAN_RINGS;
        }
        return Math.max(1, Math.min(16, scanRings.getValue()));
    }

    private double currentScanRange() {
        if (scanRange == null) {
            return DYNAMIC_SCAN_DISTANCE;
        }
        return Math.max(2.0D, Math.min(10.0D, (double) scanRange.getValue()));
    }

    private long currentFadeMs() {
        if (fadeMs == null) {
            return ARROW_FADE_MS;
        }
        return Math.max(0, Math.min(1000, fadeMs.getValue()));
    }
    ...
    private Float findStaticVoidYaw(EntityPlayer target) {
        ...
        int dirs = currentScanDirs();
        int rings = currentScanRings();
        ...
        for (int ring = 1; ring <= rings; ring++) {
            ...
            for (int i = 0; i < dirs; i++) {
                double angle = Math.PI * 2.0D * (double) i / (double) dirs;
                double dirX = Math.cos(angle);
                double dirZ = Math.sin(angle);
                double x = target.posX + dirX * radius;
                double z = target.posZ + dirZ * radius;
    ...
    private double scoreVoidPath(EntityPlayer target, double forwardX, double forwardZ) {
        ...
        double scanDistance = currentScanRange();
        AxisAlignedBB baseCollisionBox =
                target
                        .getEntityBoundingBox()
                        .contract(DYNAMIC_COLLISION_INSET, 0.0D, DYNAMIC_COLLISION_INSET);

        for (int step = 1; step <= (int) (scanDistance / DYNAMIC_SCAN_STEP); step++) {
            ...
            for (int side = -1; side <= 1; side++) {
                ...
                if (isVoidColumn(x, target.posY, z)) {
                    ...
                    score += laneWeight * (scanDistance + DYNAMIC_SCAN_STEP - forward);
                    centerVoid |= side == 0;
                }
            }
    ...
    private void startArrowFade() {
        ...
        long fadeWindowMs = currentFadeMs();
        if (lastRenderedDisplaceYaw != null
                && lastRenderedTarget != null
                && !lastRenderedTarget.isDead
                && fadeWindowMs > 0
                && nowMs - lastRenderedArrowMs <= fadeWindowMs) {
    ...
    public void onRenderWorld(Render3DEvent e) {
        ...
        if (activeArrow) {
            ...
            alpha = 1.0F - (float) fadeElapsedMs / (float) fadeWindowMs;
        }
    ...
    public void onPostInput(MoveInputEvent e) {
        ...
        if (hasKnockback.getValue() && !hasKB) return;
    ...
    public void onSendPacket(PacketEvent e) {
        ...
        if (!blink.getValue() || !active || !displaceThisTick) {
            return;
        }
    ...
    public void onUpdateLowest(UpdateEvent e) {
        ...
        boolean attacking =
                mc.gameSettings.keyBindAttack.isKeyDown()
                        || (OpenSkid.moduleManager.modules.get(KillAura.class) != null
                        && OpenSkid.moduleManager.modules.get(KillAura.class).isEnabled()
                        && ((KillAura) OpenSkid.moduleManager.modules.get(KillAura.class)).getTarget() != null);
        if (attacking) {
