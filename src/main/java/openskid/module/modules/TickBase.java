...
public class TickBase extends Module {
    ...
    // Reentrancy guard: runExtraTicks drives player updates that can reenter onTick.
    private boolean ticking;
    private final List<TickData> tickBuffer = new ArrayList<>();

    public TickBase() {
        super("TickBase", false, false, "Stores ticks to burst toward enemies in range.");
    }
    ...
    private boolean isKillAuraActive() {
        Module module = OpenSkid.moduleManager.modules.get(KillAura.class);
        return module != null && module.isEnabled();
    }

    private boolean isBlinkEnabled() {
        if (OpenSkid.blinkManager != null && OpenSkid.blinkManager.isBlinking()) {
            return true;
        }
        Module module = OpenSkid.moduleManager.modules.get(Blink.class);
        return module != null && module.isEnabled();
    }
