...
public class TargetStrafe extends Module {
    ...
    public final BooleanProperty allowFly = new BooleanProperty("allow-fly", false);
    public final BooleanProperty allowManual = new BooleanProperty("allow-manual", false);
    public final BooleanProperty strafeAround = new BooleanProperty("strafe-around", true);
    public final FloatProperty baseSpeed = new FloatProperty("base-speed", 0.29F, 0.10F, 1.00F);
    public final BooleanProperty standalone = new BooleanProperty("standalone", true);
    public final ModeProperty directionMode = new ModeProperty("direction", 0, new String[]{"AUTO", "LEFT", "RIGHT"});

    private boolean isMoverActive() {
        Fly fly = (Fly) OpenSkid.moduleManager.modules.get(Fly.class);
        Speed speed = (Speed) OpenSkid.moduleManager.modules.get(Speed.class);
        ...
        return longJump != null && longJump.isEnabled() && longJump.isJumping();
    }

    private boolean canStrafe() {
        Fly fly = (Fly) OpenSkid.moduleManager.modules.get(Fly.class);
        if (!this.allowFly.getValue() && fly.isEnabled()) {
            return false;
        }
        if (this.speedOnly.getValue() && !this.allowManual.getValue() && !this.isMoverActive()) {
            if (!this.standalone.getValue()) {
                return false;
            }
        }
    ...
    private Color getTargetColor(EntityLivingBase entityLivingBase) {
        ...
        switch (this.showTarget.getValue()) {
            ...
            case 2:
                int color = ((HUD) OpenSkid.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis());
    ...
    public TargetStrafe() {
        super("TargetStrafe", false, false, "Strafes in a circle around your combat target.");
    }
    ...
    public void onUpdate(UpdateEvent event) {
        if (this.isEnabled() && event.getType() == EventType.PRE) {
            // Ported from Raven TargetStrafe.java: manual direction plus collide/water flip.
            if (this.directionMode.getValue() == 0) {
                ...
                this.direction = this.directionMode.getValue() == 1 ? 1 : -1;
            }
            if (!this.canStrafe()) {
                this.target = null;
                ...
                if (this.target == null) {
                    ...
                    if (vpositions.isEmpty()) {
                        ...
                        if (mc.thePlayer.isCollidedHorizontally && this.strafeAround.getValue()) {
                            this.direction *= -1;
                        }
                        ...
                        if (this.isInWater(nextX, nextZ) && this.strafeAround.getValue()) {
                            this.direction *= -1;
    ...
    public void onStrafe(StrafeEvent event) {
        if (this.isEnabled()) {
            if (!Float.isNaN(this.targetYaw) && MoveUtil.isForwardPressed()) {
                ...
                if (this.standalone.getValue() && !this.isMoverActive() && mc.thePlayer != null) {
                    MoveUtil.setSpeed(this.baseSpeed.getValue().doubleValue(), this.targetYaw);
                }
            }
        }
    }
