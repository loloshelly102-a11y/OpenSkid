...
import openskid.events.SwapItemEvent;
...
public class AutoBedDef extends Module {
    ...
    // Swap-back plus own-bed radius plus defense tiers. Tier idea from donor BedDefender neighbor cover.
    public final BooleanProperty swapBack = new BooleanProperty("swap-back", true);
    public final FloatProperty ownBedRadius = new FloatProperty("own-bed-radius", 8.0f, 3.0f, 16.0f);
    public final ModeProperty defenseTier = new ModeProperty("defense-tier", 1, new String[]{"BASIC", "ROOF", "FORT"});
    ...
    public AutoBedDef() {
        super("AutoBedDef", false, false, "Automatically builds wool defense around your bed.");
    }
    ...
    public void onEnabled() {
        if (mc.thePlayer != null) {
            ...
            lastSlot = ScaffoldSessionState.saveSlotOnce(-1, mc.thePlayer.inventory.currentItem);
        }
        progress = 0f;
        resetTarget();
        lockedBedFoot = null;
    ...
    public void onDisabled() {
        if (swapBack.getValue() && ScaffoldSessionState.hasSavedSlot(lastSlot) && mc.thePlayer != null && mc.thePlayer.inventory.currentItem != lastSlot) {
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (nextIdx == -1) {
            resetTarget();
            if (swapBack.getValue() && ScaffoldSessionState.hasSavedSlot(lastSlot) && mc.thePlayer.inventory.currentItem != lastSlot) {
                mc.thePlayer.inventory.currentItem = lastSlot;
            }
            return;
        }
    ...
    private void findNearestBed() {
        ...
        double maxDistSq = (double) ownBedRadius.getValue() * (double) ownBedRadius.getValue();
    ...
    private List<BlockPos> getDefensePositions() {
        ...
        if (defenseTier.getValue() <= 0) {
            return list;
        }
        list.add(foot.up(1));
        list.add(head.up(1));
        if (defenseTier.getValue() <= 1) {
            return list;
        }
