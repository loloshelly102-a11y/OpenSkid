...
import openskid.util.rotation.Rotation;
...
public class KillAura extends Module {
    ...
    public final BooleanProperty humanizer;
    public final PercentProperty missChance;
    public final IntProperty switchPauseMs;
    private long lastTargetSwitchMs = 0L;
    ...
    public final IntProperty grimBlockTicks;
    public final BooleanProperty watchdogRelease;
    public final FloatProperty polarTurnSpeed;
    public final FloatProperty polarPredict;
    public final FloatProperty intaveBias;
    public final BooleanProperty keepRangeEnable;
    public final FloatProperty keepRangeDist;
    public final ModeProperty keepRangeMode;
    public final BooleanProperty smoothAimEnable;
    ...
    public KillAura() {
        ...
        this.autoBlock = new ModeProperty("auto-block", 3, new String[]{"NONE", "VANILLA", "SPOOF", "HYPIXEL", "BLINK", "INTERACT", "SWAP", "LEGIT", "FAKE", "Morden", "GRIMAC-1.8", "GRIMAC-1.12", "WATCHDOG"});
        ...
        this.humanizer = new BooleanProperty("Humanizer", false);
        this.missChance = new PercentProperty("MissChance", 5, this.humanizer::getValue);
        this.switchPauseMs = new IntProperty("SwitchPauseMs", 150, 0, 1000, this.humanizer::getValue);
        this.rotations = new ModeProperty("Rotations", 2, new String[]{"NONE", "Legit", "Silent", "LockView", "LiquidBounce", "Hypixel", "Polar", "Intave"});
        ...
        this.grimBlockTicks = new IntProperty("GrimBlockTicks", 1, 0, 5, () -> this.autoBlock.getValue() == 10 || this.autoBlock.getValue() == 11);
        this.watchdogRelease = new BooleanProperty("WatchdogRelease", true, () -> this.autoBlock.getValue() == 12);
        this.polarTurnSpeed = new FloatProperty("PolarTurnSpeed", 90.0F, 10.0F, 180.0F, () -> rotations.getValue() == 6);
        this.polarPredict = new FloatProperty("PolarPredict", 0.5F, 0.0F, 1.0F, () -> rotations.getValue() == 6);
        this.intaveBias = new FloatProperty("IntaveBias", 2.0F, 0.0F, 5.0F, () -> rotations.getValue() == 7);
        this.keepRangeEnable = new BooleanProperty("KeepRange", false);
        this.keepRangeDist = new FloatProperty("KeepRangeDist", 2.8F, 1.0F, 4.0F, () -> this.keepRangeEnable.getValue());
        this.keepRangeMode = new ModeProperty("KeepRangeMode", 0, new String[]{"Back", "Stop"}, () -> this.keepRangeEnable.getValue());
        this.smoothAimEnable = new BooleanProperty("SmoothAim", false);
    ...
    private boolean performAttack(float yaw, float pitch) {
        if (!OpenSkid.playerStateManager.digging && !OpenSkid.playerStateManager.placing) {
            if (this.isPlayerBlocking() && this.autoBlock.getValue() != 1) {
                return false;
            } else if (this.attackDelayMS > 0L) {
                return false;
            } else if (this.humanizer.getValue()
                    && System.currentTimeMillis() - this.lastTargetSwitchMs < (long) this.switchPauseMs.getValue()) {
                return false;
            } else if (this.humanizer.getValue()
                    && Math.random() * 100.0 < (double) this.missChance.getValue()) {
    ...
    private float threatScore(EntityLivingBase entity) {
        float score = 0.0F;
        if (entity instanceof net.minecraft.entity.player.EntityPlayer) {
            ...
            if (held != null) {
                score += (float) ItemUtil.getAttackBonus(held);
            }
            score += (float) player.getTotalArmorValue() * 0.5F;
            score += (20.0F - player.getHealth()) * 0.25F;
        } else {
            score += (20.0F - entity.getHealth()) * 0.25F;
        }
        return score;
    }

    private boolean canAttack() {        if (this.inventoryCheck.getValue() && mc.currentScreen instanceof GuiContainer) {
            return false;
        } else if (this.gapplePause.getValue() && mc.thePlayer != null && mc.thePlayer.isUsingItem() && isHoldingGapple()) {
            // Ported from OldGrimGapple.java: yield attacks while gapple-eating.
            return false;
            ...
            if (((IAccessorPlayerControllerMP) mc.playerController).getIsHittingBlock()) {
                ...
                AutoHeal autoHeal = (AutoHeal) OpenSkid.moduleManager.modules.get(AutoHeal.class);
                if (autoHeal.isEnabled() && autoHeal.isSwitching()) {
                    ...
                    if (bedNuker.isEnabled() && bedNuker.isReady()) {
                        return false;
                    } else if (OpenSkid.moduleManager.modules.get(Scaffold.class).isEnabled()) {
                        return false;
    ...
    private boolean isHoldingGapple() {
        ...
        return held != null && held.getItem() instanceof ItemAppleGold;
    }
    ...
    public void onUpdate(UpdateEvent event) {
        if (event.getType() == EventType.POST && this.blinkReset) {
            ...
            OpenSkid.blinkManager.setBlinkState(true, BlinkModules.AUTO_BLOCK);
        }
                ...
                if (attack) {
                        ...
                        switch (this.autoBlock.getValue()) {
                            case 0:
                                if (PlayerUtil.isUsingItem()) {
                                    ...
                                    if (!this.isPlayerBlocking() && !OpenSkid.playerStateManager.digging && !OpenSkid.playerStateManager.placing) {
                                        swap = true;
                                    }
                                } else {
                                    this.isBlocking = false;
                            ...
                            case 2:
                                if (this.hasValidTarget()) {
                                    ...
                                    if (OpenSkid.playerStateManager.digging || OpenSkid.playerStateManager.placing || mc.thePlayer.inventory.currentItem != item || this.isPlayerBlocking() && this.blockTick != 0 || this.attackDelayMS > 0L && this.attackDelayMS <= 50L) {
                                        this.blockTick = 0;
                                    } else {
                                ...
                                if (this.hasValidTarget()) {
                                    if (!OpenSkid.playerStateManager.digging && !OpenSkid.playerStateManager.placing) {
                                        switch (this.blockTick) {
                                            ...
                                            case 1:
                                                if (this.isPlayerBlocking()) {
                                                    if (OpenSkid.moduleManager.modules.get(NoSlow.class).isEnabled()) {
                            ...
                            case 5:
                                if (this.hasValidTarget()) {
                                    int item = ((IAccessorPlayerControllerMP) mc.playerController).getCurrentPlayerItem();
                                    if (mc.thePlayer.inventory.currentItem == item && !OpenSkid.playerStateManager.digging && !OpenSkid.playerStateManager.placing) {
                                        switch (this.blockTick) {
                                            case 0:
                                                if (!this.isPlayerBlocking()) {
                                                    swap = true;
                            ...
                            case 8:
                                ...
                                if (PlayerUtil.isUsingItem() && !this.isPlayerBlocking() && !OpenSkid.playerStateManager.digging && !OpenSkid.playerStateManager.placing) {
                                    swap = true;
                                }
                                break;
                            case 10:
                                // Ported from GrimAC18AutoBlock.java: hold block state, real use via controller tick.
                                if (this.hasValidTarget()) {
                                    if (!this.isPlayerBlocking() && !OpenSkid.playerStateManager.digging && !OpenSkid.playerStateManager.placing) {
                                        if (this.grimBlockTicks.getValue() <= 0 || this.blockTick % (this.grimBlockTicks.getValue() + 1) == 0) {
                                            swap = true;
                                        }
                                        this.blockTick++;
                                    }
                                ...
                                break;
                            case 11:
                                // Ported from GrimAC112AutoBlock.java: confirm-transaction + swing before block.
                                if (this.hasValidTarget()) {
                                    if (ItemUtil.isHoldingSword() && !OpenSkid.playerStateManager.digging && !OpenSkid.playerStateManager.placing) {
                                        PacketUtil.sendPacket(new C0FPacketConfirmTransaction(random.nextInt(Integer.MAX_VALUE - 1) + 1, (short) random.nextInt(32767), true));
                                ...
                                break;
                            case 12:
                                // Ported from WatchdogAutoBlock.java: plain swap block with optional release.
                                if (this.hasValidTarget()) {
                                    if (!this.isPlayerBlocking() && !OpenSkid.playerStateManager.digging && !OpenSkid.playerStateManager.placing) {
                                        swap = true;
                                    }
                                    ...
                                    if (this.watchdogRelease.getValue() && this.isPlayerBlocking()) {
                                        this.stopBlock();
                                    }
                                    OpenSkid.blinkManager.setBlinkState(false, BlinkModules.AUTO_BLOCK);
                                    this.isBlocking = false;
                    ...
                    if (this.target != null && this.isBoxInSwingRange(this.target.getBox())) {
                        if (this.rotations.getValue() == 4) {
                            ...
                            // Ported from PolarRotation.java: trimmed predicted box plus small gaussian noise.
                            AxisAlignedBB aimBox = getPolarAimBox(this.target.getEntity());
                            float[] rotations = RotationUtil.getRotationsToBox(
                                    aimBox,
                                    event.getYaw(),
                                    event.getPitch(),
                                    Math.min((float) this.angleStep.getValue(), this.polarTurnSpeed.getValue()),
                                    (float) this.smoothing.getValue() / 100.0F
                            );
                            float finalYaw = (float) (rotations[0] + random.nextGaussian() * 0.15);
                            float finalPitch = (float) (rotations[1] + random.nextGaussian() * 0.15);
                            float[] clamped = clampSmoothAim(finalYaw, finalPitch, event.getYaw(), event.getPitch());
                            finalYaw = clamped[0];
                            finalPitch = clamped[1];
                        ...
                        } else if (this.rotations.getValue() == 7) {
                            // Ported from IntaveRotation.java: drifting bias plus oscillation around the box aim.
                            float[] rotations = RotationUtil.getRotationsToBox(
                                    this.target.getBox(),
                                    event.getYaw(),
                                    event.getPitch(),
                            ...
                            float bias = this.intaveBias.getValue();
                            float wobble = (float) Math.sin(System.currentTimeMillis() / 220.0) * 0.6F;
                            float finalYaw = rotations[0] + RandomUtil.nextFloat(-bias, bias) * 0.5F + wobble * 0.4F;
                            float finalPitch = rotations[1] + RandomUtil.nextFloat(-bias, bias) * 0.4F;
                            float[] clamped = clampSmoothAim(finalYaw, finalPitch, event.getYaw(), event.getPitch());
                            finalYaw = clamped[0];
                            finalPitch = clamped[1];
                            float[] fixed = RotationUtil.gcd(new float[]{finalYaw, finalPitch}, new float[]{event.getYaw(), event.getPitch()});
                            ...
                            if (this.rotations.getValue() == 3) {
                                OpenSkid.rotationManager.setRotation(finalYaw, finalPitch, 1, true);
                            } else {
                                mc.thePlayer.rotationYawHead = finalYaw;
                                mc.thePlayer.renderYawOffset = finalYaw;
                            }
    ...
    private AxisAlignedBB getPolarAimBox(EntityLivingBase entity) {
        double playerSpeed = Math.hypot(mc.thePlayer.motionX, mc.thePlayer.motionZ);
        double targetSpeed = Math.hypot(entity.posX - entity.lastTickPosX, entity.posZ - entity.lastTickPosZ);
        double trim = Math.min(0.15, (playerSpeed + targetSpeed) * 0.2);
        AxisAlignedBB bb = entity.getEntityBoundingBox().expand(-trim, -Math.min(0.2, trim), -trim);
        double k = this.polarPredict.getValue();
        double dx = (entity.posX - entity.lastTickPosX) * k;
        double dy = (entity.posY - entity.lastTickPosY) * k;
        double dz = (entity.posZ - entity.lastTickPosZ) * k;
        return bb.offset(dx, dy, dz);
    }

    private float[] clampSmoothAim(float yaw, float pitch, float curYaw, float curPitch) {
        // Ported from SmoothAimAssist.java: per-tick horizontal/vertical speed caps.
        if (!this.smoothAimEnable.getValue()) {
            return new float[]{yaw, pitch};
        }
        float yawDiff = MathHelper.wrapAngleTo180_float(yaw - curYaw);
        float pitchDiff = MathHelper.wrapAngleTo180_float(pitch - curPitch);
        yawDiff = MathHelper.clamp_float(yawDiff, -this.smoothAimH.getValue(), this.smoothAimH.getValue());
        pitchDiff = MathHelper.clamp_float(pitchDiff, -this.smoothAimV.getValue(), this.smoothAimV.getValue());
        return new float[]{curYaw + yawDiff, MathHelper.clamp_float(curPitch + pitchDiff, -90.0F, 90.0F)};
    }
                    ...
                    if (validTargets.isEmpty()) {
                        ...
                        validTargets.sort(
                                (entityLivingBase1, entityLivingBase2) -> {
                                    ...
                                    switch (this.sort.getValue()) {
                                        ...
                                        case 4:
                                            sortBase = Float.compare(
                                                    threatScore(entityLivingBase2),
                                                    threatScore(entityLivingBase1)
                                            );
                                            break;
                                    }
                                    return sortBase != 0
                        ...
                        if (needsNewTarget) {
                            ...
                            if (targetChanged) {
                                ...
                                this.lastTargetSwitchMs = System.currentTimeMillis();
                            }
                        }
                    }
                    if (this.target != null) {
    ...
    public void onMove(MoveInputEvent event) {
        if (this.isEnabled() || this.wantsToDisable) {
            ...
            boolean isPolar = this.rotations.getValue() == 6;
            boolean isIntave = this.rotations.getValue() == 7;
            if (this.moveFix.getValue() != 0 && (isSilent || isLiquidBounce || isRavenBS || isPolar || isIntave)) {
            ...
            // Ported from KeepRange.java: hold spacing when too close to the aura target.
            if (this.keepRangeEnable.getValue() && this.target != null && this.isEnabled()) {
                double dist = RotationUtil.distanceToEntity(this.target.getEntity());
                if (dist < (double) this.keepRangeDist.getValue() - 0.05) {
                    if (this.keepRangeMode.getValue() == 1) {
