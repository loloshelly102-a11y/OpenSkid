...
// Placement flow adapted from project-local AutoBedDef. Neighbor-cover idea from donor BedDefender.
public class BedDefender extends Module {
    ...
    public final IntProperty refillMin = new IntProperty("refill-min", 8, 0, 64, () -> refillCheck.getValue());
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (refillCheck.getValue() && countBlocks() < refillMin.getValue()) {
            resetTarget();
            return;
        }
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        String text = String.format("Defending: %.0f%%", progress * 100.0f);
    ...
    private void findNearestBed() {
        ...
        int searchRadius = Math.max(1, Math.round(bedRadius.getValue()));
        double maxDistSq = (double) bedRadius.getValue() * (double) bedRadius.getValue();
