...
import openskid.util.shader.BlurShader;
import openskid.util.shader.BlurUtils;
import openskid.util.shader.RoundedUtils;
import openskid.util.shader.ShadowShader;
...
public class TargetHUD extends Module {
    ...
    private static final int OPENSKID_STYLE_START = 8;
    private static final int COMBO_STYLE_START = 15;

    public final ModeProperty style = new ModeProperty("style", 0, new String[]{
            "DEFAULT", "SKID-MODERN", "SKID-LEGACY", "FACE", "THREED", "SIMPLE", "CIRCLE", "AZURE",
            "PASTEL", "GRAND", "ECLIPSE", "CARDS", "FROST", "PULSE", "Pastel",
            "GRAND-X", "SKID-NEW", "PASTEL-OLD"
    });
    public final ModeProperty color = new ModeProperty("color", 0, new String[]{"DEFAULT", "HUD"}, this::isOpenSkidStyle);
    public final ModeProperty animMode = new ModeProperty("Anim Mode", 0, new String[]{"ELASTIC", "SCALE"}, this::isOpenSkidPlusStyle);
    public final ModeProperty colorMode = new ModeProperty("Color", 0, new String[]{"SYNC", "CUSTOM", "HEALTH", "PASTEL"}, this::isOpenSkidPlusStyle);
    public final ColorProperty customColor = new ColorProperty("CustomColor", new Color(255, 50, 50).getRGB() & 0xFFFFFF, () -> isOpenSkidPlusStyle() && colorMode.getValue() == 1);
    public final IntProperty bgAlpha = new IntProperty("Bg-Alpha", 180, 0, 255, () -> isOpenSkidPlusStyle() && getOpenSkidPlusStyle() != 1);
    ...
    public final BooleanProperty outline = new BooleanProperty("outline", false, () -> isOpenSkidStyle() && (this.style.getValue() == 0 || this.style.getValue() == 1 || this.style.getValue() >= 3));
    ...
    public final ModeProperty comboFont = new ModeProperty("Combo-Font", 0, new String[]{"TAHOMA", "PULSE", "REGULAR"}, () -> this.style.getValue() >= COMBO_STYLE_START);
    public final BooleanProperty comboShadow = new BooleanProperty("Combo-Shadow", true, () -> this.style.getValue() >= COMBO_STYLE_START);
    public final BooleanProperty comboStatus = new BooleanProperty("Combo-Status", true, () -> this.style.getValue() == COMBO_STYLE_START + 1);
    ...
    private String sizeCacheName;
    private int sizeCacheStyle = -1;
    private float sizeCacheWidth;
    private float sizeCacheHeight;

    public TargetHUD() {
        super("TargetHUD", false, true, "Shows target health, name and armor on screen.");
    }

    private boolean isOpenSkidStyle() {
        return this.style.getValue() < OPENSKID_STYLE_START;
    }

    private boolean isOpenSkidPlusStyle() {
        return this.style.getValue() >= OPENSKID_STYLE_START;
    }

    private int getOpenSkidPlusStyle() {
        return this.style.getValue() - OPENSKID_STYLE_START;
    }
    ...
    public void onRender(Render2DEvent event) {
        ...
        if (isOpenSkidPlusStyle()) {
            renderOpenSkidPlusTargetHud();
        } else {
            renderOpenSkidTargetHud();
        }
    }

    private void renderOpenSkidTargetHud() {
    ...
    private void drawClassicPanelStyle(EntityLivingBase entity, boolean forceHead, boolean prefer3d, int hudWidth, int hudHeight) {
        ...
        int[] classicPos = getClassicPosition(hudWidth, hudHeight);
        int baseX = classicPos[0];
        int baseY = classicPos[1];
    ...
    private void drawSimpleStyle(EntityLivingBase entity) {
        float alpha = Math.min(1.0F, this.animatedScale);
        int[] classicPos = getClassicPosition(130, 32);
        int baseX = classicPos[0];
        int baseY = classicPos[1];
        int hudWidth = 130;
        int hudHeight = 32;
    ...
    private void drawCircleStyle(EntityLivingBase entity) {
        float alpha = Math.min(1.0F, this.animatedScale);
        int[] classicPos = getClassicPosition(120, 40);
        int baseX = classicPos[0];
        int baseY = classicPos[1];
    ...
    private void drawAzureStyle(EntityLivingBase entity) {
        float alpha = Math.min(1.0F, this.animatedScale);

        int[] classicPos = getClassicPosition(165, 55);
        int x = classicPos[0];
        int y = classicPos[1];
    ...
    private void renderOpenSkidPlusTargetHud() {
        ...
        float[] size = getOpenSkidPlusStyleSize();
        float width = size[0];
        float height = size[1];
        float[] pos = getOpenSkidPlusPosition(width, height);
        ...
        boolean useBlur = shouldUseHudBlur() && animValue > 0.05;
        boolean useShadow = shouldUseHudShadow();
        boolean useGlow = shouldUseHudGlow() && animValue > 0.05;
        ...
        switch (getOpenSkidPlusStyle()) {
            case 0:
                renderPastel(pos[0], pos[1], width, height, renderHealth, useShadow);
                break;
            case 1:
                renderGrand(pos[0], pos[1], width, height, renderHealth, useShadow);
                break;
            case 2:
                renderMoon(pos[0], pos[1], width, height, renderHealth, useShadow);
                break;
            case 3:
                renderCard(pos[0], pos[1], width, height, renderHealth, useShadow);
                break;
            case 4:
                renderFrost(pos[0], pos[1], renderHealth, useShadow);
                break;
            case 5:
                renderPulse(pos[0], pos[1], width, height, renderHealth, useShadow);
                break;
            case 6:
                renderPastel(pos[0], pos[1], width, height, renderHealth, useShadow);
                break;
            case 7:
                renderGrandX(pos[0], pos[1], width, height, renderHealth, useShadow);
                break;
            case 8:
                renderRavenNew(pos[0], pos[1], width, height, renderHealth, useShadow);
                break;
            case 9:
                renderOldPastel(pos[0], pos[1], width, height, renderHealth, useShadow);
                break;
            default:
                break;
        }
    ...
    private void renderCard(float x, float y, float width, float height, float health, boolean shadow) {
        GlStateManager.pushMatrix();
        ...
        if (healthPct > 0.01F) {
            ...
            drawGradientRoundedRect(42, 22.0F, displayW, 6, 3, getOpenSkidPlusColor(0), getOpenSkidPlusColor((int) (displayW * 2)));
        }

        drawOpenSkidPlusFace(renderTarget, 4.0F, 3.3F, 33, 33, 6);
        FontManager.productSans18.drawString(String.format("%.1f", health), width - 30, 9, getOpenSkidPlusColor(0).getRGB(), true);
        FontManager.productSans18.drawString(renderTarget.getName(), 42, 9, -1, true);
        GlStateManager.popMatrix();
    }

    private void renderPastel(float x, float y, float width, float height, float health, boolean shadow) {
        GlStateManager.pushMatrix();
        GlStateManager.translate(x, y, 0.0F);
        if (shadow) {
        ...
        FontManager.regular18.drawString(String.format("%.1f", health) + " \u2764", 50 / 1.5F, 22 / 1.5F, getOpenSkidPlusColor(0).getRGB(), true);
        GlStateManager.popMatrix();

        float healthPct = MathHelper.clamp_float(health / renderTarget.getMaxHealth(), 0, 1);
        float barWidth = width - 54;
        RenderUtil.drawRect(48, 42, 48 + barWidth, 49, ColorUtil.darker(getOpenSkidPlusColor(0), 0.3F).getRGB());

        drawHorizontalGradientRect(48, 42, barWidth * healthPct, 7, getOpenSkidPlusColor(0).getRGB(), getOpenSkidPlusColor((int) (barWidth * healthPct * 2)).getRGB());
        GlStateManager.popMatrix();
    }

    private void renderGrand(float x, float y, float width, float height, float health, boolean shadow) {
        GlStateManager.pushMatrix();
        GlStateManager.translate(x, y, 0.0F);
        if (shadow) {
        ...
        drawGrandBorderedRect(-2.5F, -2.5F, width + 2.5F, height + 2.5F, 0.5F, getGrandColor(60), getGrandColor(10));
        drawGrandBorderedRect(-1.5F, -1.5F, width + 1.5F, height + 1.5F, 1.5F, getGrandColor(60), getGrandColor(40));
        drawGrandBorderedRect(0, 0, width, height, 0.5F, getGrandColor(22), getGrandColor(60));
        drawGrandBorderedRect(2, 2, 38, 38, 0.5F, getGrandColor(0, 0), getGrandColor(10));
        drawGrandBorderedRect(2.5F, 2.5F, 37.5F, 37.5F, 0.5F, getGrandColor(17), getGrandColor(48));
        ...
        RenderUtil.drawRect(42, 12, width - 8, 16, getGrandColor(0, 0));
    ...
    private void renderMoon(float x, float y, float width, float height, float health, boolean shadow) {
        ...
        if (pct > 0.01F) {
            ...
            drawGradientRoundedRect(42, 26.5F, displayW, 8.5F, 4, getOpenSkidPlusColor(0), getOpenSkidPlusColor((int) (displayW * 2)));
        }

        drawOpenSkidPlusFace(renderTarget, 2.5F, 2.5F, 35, 35, 8);
        FontManager.pulse12.drawString(String.format("%.1f", health) + "HP", 40, 17, -1, true);
        FontManager.pulse16.drawString(renderTarget.getName(), 40, 6, -1, true);
        GlStateManager.popMatrix();
    }

    private void renderFrost(float x, float y, float health, boolean shadow) {
        float width = Math.max(125.0F, (float) (FontManager.pulse16.getStringWidth(renderTarget.getName()) + 42));
        float height = 32.5F;
        GlStateManager.pushMatrix();
        GlStateManager.translate(x, y, 0.0F);
        if (shadow) {
        ...
        drawOpenSkidPlusFace(renderTarget, 3, 3, 26, 26, 4);

        float circleX = width - 15.0F;
        drawCircle(circleX, new Color(0, 0, 0, 100).getRGB());
        drawArc(circleX, 360 * MathHelper.clamp_float(health / renderTarget.getMaxHealth(), 0, 1), getOpenSkidPlusColor(0).getRGB());

        FontManager.pulse16.drawString(renderTarget.getName(), 34, 8, -1, true);
        FontManager.pulse12.drawString("Dist: " + String.format("%.1f", renderTarget.getDistanceToEntity(mc.thePlayer)) + "m", 34, 20, getOpenSkidPlusColor(0).getRGB(), true);
        GlStateManager.popMatrix();
    }

    private void renderPulse(float x, float y, float width, float height, float health, boolean shadow) {
        GlStateManager.pushMatrix();
        GlStateManager.translate(x, y, 0.0F);
        if (shadow) {
        ...
        drawOpenSkidPlusFace(renderTarget, 4, 4, 34, 34, 6);
        FontManager.pulse20.drawString(renderTarget.getName(), 43, 10, -1, true);
        FontManager.pulse12.drawString("HP: " + String.format("%.1f", health), 43, 20, -1, true);
        ...
        if (pct > 0.01F) {
            ...
            drawGradientRoundedRect(44, 30, displayW, 6, 3, getOpenSkidPlusColor(0), getOpenSkidPlusColor((int) (displayW * 2)));
        }
        GlStateManager.popMatrix();
    }

    private FontRenderer getComboFont() {
        switch (comboFont.getValue()) {
            case 1:
                return FontManager.pulse16;
            case 2:
                return FontManager.regular18;
            default:
                return FontManager.tahomaBold16;
        }
    }

    private void renderGrandX(float x, float y, float width, float height, float health, boolean shadow) {
        // Adapted from the donor Grand bordered-panel concept.
        GlStateManager.pushMatrix();
        GlStateManager.translate(x, y, 0.0F);
        if (shadow) {
            ShadowShader.drawShadow(-2.5F, -2.5F, width + 5.0F, height + 5.0F, 4, 4, new Color(0, 0, 0, 100).getRGB());
        ...
        drawOpenSkidPlusFace(renderTarget, 4, 4, 34, 34, 4);
        FontRenderer font = getComboFont();
        boolean fontShadow = comboShadow.getValue();
        font.drawString(renderTarget.getName(), 44, 5, -1, fontShadow);

        float pct = MathHelper.clamp_float(health / renderTarget.getMaxHealth(), 0, 1);
        Color hpColor = blendColors(new float[]{0.0F, 0.5F, 1.0F}, new Color[]{Color.RED, Color.YELLOW, Color.GREEN}, pct);
        float barX = 44;
        float barY = 19;
        float barW = width - barX - 8;
        float barH = 7;
        RenderUtil.drawRect(barX, barY, barX + barW, barY + barH, getGrandColor(0, 0));
        RenderUtil.drawRect(barX, barY, barX + barW * pct, barY + barH, hpColor.getRGB());
        for (float tick = barX + 10; tick < barX + barW; tick += 10) {
            RenderUtil.drawRect(tick, barY, tick + 1, barY + barH, getGrandColor(0, 160));
        }

        int armorValue = renderTarget instanceof EntityPlayer ? ((EntityPlayer) renderTarget).getTotalArmorValue() : 0;
        float armorPct = MathHelper.clamp_float(armorValue / 20.0F, 0, 1);
        RenderUtil.drawRect(barX, barY + barH + 3, barX + barW, barY + barH + 6, getGrandColor(0, 0));
        RenderUtil.drawRect(barX, barY + barH + 3, barX + barW * armorPct, barY + barH + 6, new Color(70, 170, 255).getRGB());

        mc.fontRendererObj.drawString("HP: " + (int) health + " | " + (int) mc.thePlayer.getDistanceToEntity(renderTarget) + "m",
                barX, barY + barH + 9, -1, fontShadow);
        GlStateManager.popMatrix();
    }

    private void renderRavenNew(float x, float y, float width, float height, float health, boolean shadow) {
        // Adapted from the donor RavenNew compact-panel concept.
        GlStateManager.pushMatrix();
        GlStateManager.translate(x, y, 0.0F);
        if (shadow) {
            ShadowShader.drawShadow(0, 0, width, height, 6, 6, new Color(0, 0, 0, 100).getRGB());
        ...
        RenderUtil.drawRoundedRect(0, 0, width, height, 6, new Color(8, 8, 8, bgAlpha.getValue()).getRGB());

        float pct = MathHelper.clamp_float(health / renderTarget.getMaxHealth(), 0, 1);
        String status = "";
        if (comboStatus.getValue() && mc.thePlayer != null) {
            float selfPct = MathHelper.clamp_float(mc.thePlayer.getHealth() / mc.thePlayer.getMaxHealth(), 0, 1);
            status = pct <= selfPct ? " §aW" : " §cL";
        }

        FontRenderer font = getComboFont();
        boolean fontShadow = comboShadow.getValue();
        String name = renderTarget.getName();
        font.drawString(name, 8, 6, -1, fontShadow);
        String hpText = String.format("%.1f", health) + status;
        font.drawString(hpText, width - 8 - (float) font.getStringWidth(hpText), 6, ColorUtil.getHealthBlend(pct).getRGB(), fontShadow);

        float barY = height - 13;
        float barW = width - 16;
        RenderUtil.drawRoundedRect(8, barY, barW, 6, 3, new Color(0, 0, 0, 150).getRGB());
        if (pct > 0.01F) {
            float displayW = Math.max(6, barW * pct);
            drawGradientRoundedRect(8, barY, displayW, 6, 3, getOpenSkidPlusColor(0), getOpenSkidPlusColor((int) (displayW * 2)));
        }
        GlStateManager.popMatrix();
    }

    private void renderOldPastel(float x, float y, float width, float height, float health, boolean shadow) {
        // Adapted from the MiauMinus old-Pastel thin-bar concept.
        GlStateManager.pushMatrix();
        GlStateManager.translate(x, y, 0.0F);
        if (shadow) {
            ShadowShader.drawShadow(0, 0, width, height, 6, 6, new Color(0, 0, 0, 120).getRGB());
        ...
        RenderUtil.drawRoundedRect(0, 0, width, height, 8, new Color(0, 0, 0, bgAlpha.getValue()).getRGB());

        drawOpenSkidPlusFace(renderTarget, 5, 5, 30, 30, 8);
        FontRenderer font = getComboFont();
        boolean fontShadow = comboShadow.getValue();
        font.drawString(renderTarget.getName(), 40, 6, -1, fontShadow);

        float pct = MathHelper.clamp_float(health / renderTarget.getMaxHealth(), 0, 1);
        float barY = 21;
        float barW = width - 48;
        RenderUtil.drawRect(40, barY, 40 + barW, barY + 3, new Color(0, 0, 0, 150).getRGB());
        if (pct > 0.0F) {
            drawHorizontalGradientRect(40, barY, barW * pct, 3, getOpenSkidPlusColor(0).getRGB(), getOpenSkidPlusColor((int) (barW * pct * 2)).getRGB());
        }

        FontManager.pulse20.drawString(String.format("%.1f", health), 40, 29, getOpenSkidPlusColor(0).getRGB(), fontShadow);
        GlStateManager.popMatrix();
    }
    ...
    private Color getOpenSkidPlusColor(int offset) {
    ...
    private float[] getOpenSkidPlusStyleSize() {
        if (renderTarget == null) {
            return new float[]{120, 40};
        }

        String name = renderTarget.getName();
        int currentStyle = getOpenSkidPlusStyle();
        if (name != null && name.equals(sizeCacheName) && currentStyle == sizeCacheStyle) {
            return new float[]{sizeCacheWidth, sizeCacheHeight};
        }
        float[] size = computeOpenSkidPlusStyleSize(name, currentStyle);
        sizeCacheName = name;
        sizeCacheStyle = currentStyle;
        sizeCacheWidth = size[0];
        sizeCacheHeight = size[1];
        return size;
    }

    private float[] computeOpenSkidPlusStyleSize(String name, int currentStyle) {
        if (name == null) {
            return new float[]{120, 40};
        }
        switch (currentStyle) {
            ...
            case 2:
                return new float[]{Math.max(110, (float) (FontManager.pulse16.getStringWidth(name) + 68)), 40.5F};
            case 3:
                return new float[]{Math.max(160, (float) (FontManager.productSans18.getStringWidth(name) + 30)), 40.5F};
            case 4:
                return new float[]{Math.max(125, (float) (FontManager.pulse16.getStringWidth(name) + 42)), 32.5F};
            case 5:
                return new float[]{Math.max(120, (float) (FontManager.pulse20.getStringWidth(name) + 50)), 44};
            case 6:
                return new float[]{Math.max(130, (float) (FontManager.regular18.getStringWidth(name) + 60)), 56};
            case 7:
                return new float[]{Math.max(140, (float) (FontManager.tahomaBold16.getStringWidth(name) + 62)), 52};
            case 8:
                return new float[]{Math.max(130, (float) (FontManager.regular18.getStringWidth(name) + 62)), 44};
            case 9:
                return new float[]{Math.max(130, (float) (FontManager.pulse16.getStringWidth(name) + 64)), 50};
            default:
                return new float[]{120, 40};
        }
    }

    private float[] getOpenSkidPlusPosition(float width, float height) {
    ...
    private int[] getClassicPosition(int w, int h) {
        float[] pos = getOpenSkidPlusPosition(w, h);
        return new int[]{Math.round(pos[0]), Math.round(pos[1])};
    }

    private void drawOpenSkidPlusFace(EntityLivingBase entity, float x, float y, float w, float h, float r) {
    ...
    private void drawGrandBorderedRect(float x1, float y1, float x2, float y2, float border, int fill, int out) {
        RenderUtil.drawRect(x1, y1, x2, y2, out);
        RenderUtil.drawRect(x1 + border, y1 + border, x2 - border, y2 - border, fill);
    }

    private int getGrandColor(int b) {
        return new Color(b, b, b, 255).getRGB();
    }

    private int getGrandColor(int b, int a) {
