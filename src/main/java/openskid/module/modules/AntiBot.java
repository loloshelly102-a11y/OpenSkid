...
public class AntiBot extends Module {
    ...
    public final BooleanProperty ground = new BooleanProperty("Ground", false);
    ...
    public AntiBot() {
        super("AntiBot", true, true, "Detects and removes fake players and server side bots.");
    }
    ...
    public void onTick(TickEvent event) {
        ...
        for (EntityPlayer bot : bots) {
            ...
            if (debug.getValue()) {
                ChatUtil.sendFormatted(String.format("%sAntiBot: &fRemoved &r%s &fdue to it being a bot.", OpenSkid.clientName, bot.getName()));
            }
        }
    }
    ...
    private void handleMatrixBot() {
        ...
        if (matrixCollectSample) {
            ...
            for (EntityPlayer bot : bots) {
                ...
                if (debug.getValue()) {
                    ChatUtil.sendFormatted(String.format("%sAntiBot/MatrixBot: &fRemoved &r%s&f. Speed: &b%.2f&f.", OpenSkid.clientName, bot.getName(), getHorizontalSpeed(bot)));
                }
            }
        }
    ...
    public boolean isBotPlayer(EntityLivingBase entity) {
        ...
        boolean fresh = player.ticksExisted < 40;
        if (ground.getValue() && !fresh && !touchedGround.contains(id)) return true;
        ...
        if (derp.getValue() && !fresh && (player.rotationPitch > 90.0F || player.rotationPitch < -90.0F)) return true;
        ...
        if (invalidGround.getValue() && !fresh && invalidGroundVL.getOrDefault(id, 0) >= 10) return true;
        if (tab.getValue() && !fresh && !isInTab(player)) return true;
    ...
    public static boolean isBot(EntityLivingBase entity) {
        AntiBot antiBot = (AntiBot) OpenSkid.moduleManager.getModule(AntiBot.class);
