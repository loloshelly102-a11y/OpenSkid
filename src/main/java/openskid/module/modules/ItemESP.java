...
public class ItemESP extends Module {
    ...
    private final LinkedHashMap<ItemData, Integer> itemMap = new LinkedHashMap<ItemData, Integer>();
    private static final Color EMERALD_COLOR = new Color(ChatColors.GREEN.toAwtColor());
    private static final Color DIAMOND_COLOR = new Color(ChatColors.AQUA.toAwtColor());
    private static final Color GOLD_COLOR = new Color(ChatColors.YELLOW.toAwtColor());
    private static final Color IRON_COLOR = new Color(ChatColors.WHITE.toAwtColor());
    private static final Color OTHER_COLOR = new Color(ChatColors.GRAY.toAwtColor());
    ...
    private Color getItemColor(int itemId) {
        if (this.isEmeraldItem(itemId)) {
            ...
            return this.isIronItem(itemId) ? IRON_COLOR : OTHER_COLOR;
        }
    }
    ...
    public ItemESP() {
        super("ItemESP", false, false, "Highlights valuable dropped items with boxes.");
    }
    ...
    public void onRender(Render3DEvent event) {
        ...
        boolean showCount = this.itemCount.getValue();
        boolean useAutoScale = this.autoScale.getValue();
        boolean drawOutline = this.outline.getValue();
        int opacityValue = this.opacity.getValue();
        for (Entity entity : TeamUtil.getLoadedEntitiesSorted()) {
            ...
            if (entity.getDistanceSqToEntity(mc.getRenderViewEntity()) > MAX_DIST_SQ) {
                continue;
            }
            ...
            if (stack.stackSize > 0) {
                ...
                if (this.shouldHighlightItem(itemId)) {
                    ...
                    Integer id = this.itemMap.get(data);
                    this.itemMap.put(data, stack.stackSize + (id == null ? 0 : id));
                }
            }
        }
            for (Entry<ItemData, Integer> itemEntry : this.itemMap.entrySet().stream().sorted((entry1, entry2) -> {
                ...
                double x = itemEntry.getKey().x - renderX;
                double y = itemEntry.getKey().y - renderY;
                double z = itemEntry.getKey().z - renderZ;
                double distance = mc.getRenderViewEntity().getDistance(itemEntry.getKey().x, itemEntry.getKey().y, itemEntry.getKey().z);
                double scale = 0.5 + 0.375 * ((Math.max(6.0, useAutoScale ? distance : 6.0) - 6.0) / 28.0);
                ...
                if (showCount) {
                    ...
                    double fontScale = -0.04375 - 0.0328125 * ((Math.max(6.0, useAutoScale ? distance : 6.0) - 6.0) / 28.0);
