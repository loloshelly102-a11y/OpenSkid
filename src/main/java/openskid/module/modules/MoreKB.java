...
// WTap modes SprintReset/MoreKB are canonical for WTap style movement gating only.
// This module is a distinct legit/packet reset. Keep separate. Do not merge.
public class MoreKB extends Module {
    ...
    public final ModeProperty mode = new ModeProperty("mode", 0, new String[]{"LEGIT", "LEGIT_FAST", "LESS_PACKET", "PACKET", "DOUBLE_PACKET",
            "LEGIT_SNEAK", "FAST", "LEGIT_BLOCK", "LEGIT_INV", "STAP"});
    ...
    public final IntProperty sTapTicks = new IntProperty("stap-ticks", 3, 1, 10, () -> this.mode.getValue() == 10);
    private boolean shouldSprintReset;
    private EntityLivingBase target;
    private int sTapLeft = 0;

    public MoreKB() {
        super("MoreKB", false, false, "Resets sprint on hit to deal more knockback.");
    ...
    public void onDisabled() {
        this.sTapLeft = 0;
        this.target = null;
        openskid.util.KeyBindUtil.updateKeyState(mc.gameSettings.keyBindBack.getKeyCode());
    }
        ...
        if (entity.hurtTime == 10) {
            switch (this.mode.getValue()) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    if (mc.thePlayer.isSneaking() && mc.thePlayer.isSprinting()) {
                        mc.thePlayer.setSprinting(false);
                        mc.thePlayer.setSprinting(true);
                    }
                    break;
                ...
                case 8:
                    ...
                    break;
                case 9:
                    this.sTapLeft = this.sTapTicks.getValue();
                    break;
            }
        }
        if (this.sTapLeft > 0) {
            this.sTapLeft--;
            openskid.util.KeyBindUtil.setKeyBindState(mc.gameSettings.keyBindBack.getKeyCode(), true);
            if (this.sTapLeft == 0) {
                openskid.util.KeyBindUtil.updateKeyState(mc.gameSettings.keyBindBack.getKeyCode());
            }
        }
