...
import openskid.util.player.TeamUtil;
...
public class BedwarUtils extends Module {
    ...
    public static boolean isHypixelBedWars() {
        try {
            ...
            try {
                ...
                if (scoreboard != null) {
                    ...
                    if (objective != null && objective.getDisplayName() != null) {
                        ...
                        if (title != null && title.toLowerCase(Locale.ROOT).contains("bed wars")) {
                            return true;
                        }
                    }
                }
            ...
            for (String line : ServerUtil.getScoreboardLines()) {
                try {
                    String stripped = EnumChatFormatting.getTextWithoutFormattingCodes(line);
                    if (stripped != null && stripped.toLowerCase(Locale.ROOT).contains("bed wars")) {
                        return true;
                    }
                } catch (Exception ignored) {
                }
    ...
    static boolean isStandaloneBedTrackerEnabled() {
        try {
            if (OpenSkid.moduleManager == null) {
                return false;
            }
            Module other = OpenSkid.moduleManager.getModule(openskid.module.modules.BedTracker.class);
    ...
    public BedwarUtils() {
        super("BedwarUtils", false, false, "Shows BedWars upgrades and tracks enemy items and invis players.");
    ...
    public void onPacket(PacketEvent event) {
        ...
        if (this.bedTracker.getValue() && !isStandaloneBedTrackerEnabled()) {
    ...
    public void onTick(TickEvent event) {
        ...
        boolean inBedWarsGame = isHypixelBedWars();
        if (this.itemTracker.getValue() && inBedWarsGame) {
        ...
        if (this.bedTracker.getValue() && inBedWarsGame && !isStandaloneBedTrackerEnabled()) {
            this.bedTrackerDelegate.onTick(event);
        }
        if (this.invisAlert.getValue() && inBedWarsGame) {
    ...
    private void scanInvisiblePlayers() {
        ...
        for (Object object : mc.theWorld.playerEntities) {
            ...
            if (suspicious && now - last > 5000L) {
                ...
                ChatUtil.display(this.getOpenSkidPrefix() + " &cSuspicious/invis player near bed: &f"
    ...
    private void sendItemTrackerMessage(String formattedPlayer, String item) {
        ...
        mc.thePlayer.addChatMessage(
                new ChatComponentText(this.getOpenSkidPrefix() + " §f" + formattedPlayer + " §fhas §a" + item));
    }

    private String getOpenSkidPrefix() {
        return ChatColors.formatColor(OpenSkid.clientName).trim();
    }
    ...
    private class BedTracker extends Module {
        ...
        public void onTick(TickEvent event) {
            if (this.isEnabled() && event.getType() == EventType.POST) {
                        ...
                        if (!this.trackedPearls.contains(enderPearl)) {
                            ...
                            if (this.alertOnPearl.getValue()) {
                                ChatUtil.display("%s%s: &fDetected &5Ender Pearl&r &e&l⚠&r",
                                        OpenSkid.clientName, this.getName());
                                pearl = true;
                            }
                            ...
                            if (cooldown == null
                                ...
                                ChatUtil.display(
                                        String.format("%s%s: %s&r &fis %d blocks away from your bed &e&l⚠&r",
                                                OpenSkid.clientName, this.getName(), text, (int) distance + 1));
                                pearl = true;
                            }
                ...
                if (marco && !BedwarUtils.isStandaloneBedTrackerEnabled()) {
                    ChatUtil.sendRaw(
                            String.format(
