...
public abstract class MixinEntityRenderer {
    ...
    @Unique private boolean openskid$freeLookRestoreRotation;
    @Unique private float openskid$freeLookYaw;
    @Unique private float openskid$freeLookPitch;
    @Unique private float openskid$freeLookPrevYaw;
    @Unique private float openskid$freeLookPrevPitch;
    ...
    private float hurtCameraEffect(float float1) {
        if (OpenSkid.moduleManager == null) {
            ...
            NoHurtCam noHurtCam = (NoHurtCam) OpenSkid.moduleManager.modules.get(NoHurtCam.class);
    ...
    private void a(
        ...
        if (OpenSkid.moduleManager != null) {
            GhostHand event = (GhostHand) OpenSkid.moduleManager.modules.get(GhostHand.class);
            if (event.isEnabled()) {
    ...
    private double v(Vec3 vec31, Vec3 vec32) {
        if (OpenSkid.moduleManager == null) {
            ...
            return OpenSkid.moduleManager.modules.get(ViewClip.class).isEnabled() ? (double) this.thirdPersonDistance : vec31.distanceTo(vec32);
        }
    }
    ...
    private Material x(Block block) {
        if (OpenSkid.moduleManager == null) {
            ...
            return OpenSkid.moduleManager.modules.get(ViewClip.class).isEnabled() ? Material.air : block.getMaterial();
        }
    }
    ...
    private boolean y(EntityLivingBase entityLivingBase, Potion potion) {
        if (potion == Potion.blindness && OpenSkid.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) OpenSkid.moduleManager.modules.get(AntiDebuff.class);
    ...
    private void orientFreeLook(float partialTicks, CallbackInfo callbackInfo) {
        if (OpenSkid.moduleManager == null || this.mc.thePlayer == null) {
            return;
        }
        FreeLook freeLook = (FreeLook) OpenSkid.moduleManager.modules.get(FreeLook.class);
        if (freeLook != null && freeLook.isFreeLooking()) {
            EntityPlayerSP player = this.mc.thePlayer;
            this.openskid$freeLookYaw = player.rotationYaw;
            this.openskid$freeLookPitch = player.rotationPitch;
            this.openskid$freeLookPrevYaw = player.prevRotationYaw;
            this.openskid$freeLookPrevPitch = player.prevRotationPitch;
            this.openskid$freeLookRestoreRotation = true;
    ...
    private void restoreFreeLook(float partialTicks, CallbackInfo callbackInfo) {
        if (this.openskid$freeLookRestoreRotation && this.mc.thePlayer != null) {
            EntityPlayerSP player = this.mc.thePlayer;
            player.rotationYaw = this.openskid$freeLookYaw;
            player.rotationPitch = this.openskid$freeLookPitch;
            player.prevRotationYaw = this.openskid$freeLookPrevYaw;
            player.prevRotationPitch = this.openskid$freeLookPrevPitch;
            this.openskid$freeLookRestoreRotation = false;
        }
    }
    ...
    private boolean c(EntityPlayerSP entityPlayerSP, Potion potion) {
        if (potion == Potion.confusion && OpenSkid.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) OpenSkid.moduleManager.modules.get(AntiDebuff.class);
