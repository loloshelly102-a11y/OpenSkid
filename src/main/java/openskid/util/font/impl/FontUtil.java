package openskid.util.font.impl;
...
public class FontUtil {
    ...
    public static Font getResource(Map<String, Font> locationMap, String location, int size) {
        ...
        try {
            ...
            System.err.println("[OpenSkid] Failed to load font: " + location);
    ...
    private static void addDevFontFiles(Set<File> files, File start, String location) {
        ...
        for (int i = 0; current != null && i < 8; i++) {
            files.add(new File(current, "src/main/resources/assets/openskid/font/" + location));
            files.add(new File(current, "assets/openskid/font/" + location));
            current = current.getParentFile();
        }
    }
}
