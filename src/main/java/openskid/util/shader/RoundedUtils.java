...
public class RoundedUtils {
    ...
    private static ShaderUtils roundedRectCardShader;
    ...
    private static void initShaders() {
        ...
        if (roundedRectCardShader == null) {
            try {
                roundedRectCardShader = new ShaderUtils("roundedRectCard");
            } catch (Exception e) {
                System.err.println("Failed to initialize roundedRectCardShader: " + e.getMessage());
            }
        }
    }
    ...
    public static void drawRoundedRectCard(final float x, final float y, final float width, final float height, final float radius, final int color, boolean leftTop, boolean rightTop, boolean rightBottom, boolean leftBottom) {
        initShaders();
        if (roundedRectCardShader == null) return;

        GL11.glPushMatrix();
        GlStateManager.pushAttrib();
        final int programId = roundedRectCardShader.programID;
        GL20.glUseProgram(programId);
        roundedRectCardShader.setUniformf("u_size", width, height);
        roundedRectCardShader.setUniformf("u_radius", radius);
        roundedRectCardShader.setUniformf("u_color", getRed(color), getGreen(color), getBlue(color), getAlpha(color));
        roundedRectCardShader.setUniformf("u_edges", leftBottom ? 1.0F : 0.0F, rightBottom ? 1.0F : 0.0F, rightTop ? 1.0F : 0.0F, leftTop ? 1.0F : 0.0F);
        GlStateManager.enableBlend();
        GlStateManager.blendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    ...
    public static void drawRoundedRectCard(final double x, final double y, final double width, final double height, final double radius, final int color) {
        drawRoundedRectCard((float) x, (float) y, (float) width, (float) height, (float) radius, color, true, true, true, true);
    }
