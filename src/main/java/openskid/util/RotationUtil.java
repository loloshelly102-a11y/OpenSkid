...
public class RotationUtil {
    ...
    // Shared aim bus helpers. Concept adapted from donor RotationHandler
    // (one central rotation helper set reused by combat modules), reimplemented
    // here on top of the existing RotationUtil primitives. Pure static, no state.

    public static float[] aimAtTarget(EntityLivingBase target, float maxAngle, float smooth) {
        ...
        float[] rots = RotationUtil.getRotationsToBox(
                ...
                maxAngle,
                smooth);
        rots[1] = MathHelper.clamp_float(rots[1], -90.0F, 90.0F);
        return rots;
    }

    public static boolean raycastValid(EntityLivingBase target, double range) {
        ...
        if (RotationUtil.distanceToEntity(target) > range) {
            return false;
        }
        return RotationUtil.rayTrace(target) == null;
    }

    public static Vec3 leadPoint(EntityLivingBase target, int ticks) {
        if (target == null) {
            return new Vec3(0.0D, 0.0D, 0.0D);
        }
        int lead = Math.max(0, ticks);
        double dx = (target.posX - target.lastTickPosX) * lead;
        double dy = (target.posY - target.lastTickPosY) * lead;
        double dz = (target.posZ - target.lastTickPosZ) * lead;
        AxisAlignedBB bb = target.getEntityBoundingBox();
        return new Vec3(
                (bb.minX + bb.maxX) / 2.0D + dx,
                (bb.minY + bb.maxY) / 2.0D + dy,
                (bb.minZ + bb.maxZ) / 2.0D + dz);
    }
}